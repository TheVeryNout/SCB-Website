...
Target content architecture should be:
...
- contact data
- donation details
- recurring CTA/event summary data
- membership CTA data

### `public/media/...`
All local migrated assets:
- images
- PDFs
- icons
- derived optimized media if needed

### `public/admin/config.yml`
Decap CMS config

## Planned Component Inventory
This was already specified and should be preserved as the component baseline:
...
- use `bun`, never `npm`
- Astro SSG first
- avoid SSR unless later forced by requirements
- preserve German IA and friendly URLs
- probably clean route for membership in final app, but keep legacy redirects
- do not hotlink Wix production assets
- download and serve assets locally
- content separated cleanly from components
- use content collections and structured data
- use privacy-aware embeds/placeholders for YouTube and Maps
- preserve dates/titles/excerpts/legal texts unless explicitly updated by user
- maintain a migration manifest with source URL, destination, and QA status

## Scraping / Migration Strategy
This must remain explicit because the user requested it.

### Scrape principles
- scrape for inventory first
- only then transform into Astro content
- keep raw capture and transformed content separate
- treat Wix widgets as data sources, not as markup to clone

### Sources by content type
- static pages:
  - raw HTML from page routes
  - human-readable extraction for text validation
- blog/news index:
  - RSS feed for canonical post inventory
- blog/news detail:
  - individual post page HTML for full body/gallery
- events:
  - event list page plus event detail pages
  - manual validation because of recurring-event quirks
- PDFs:
  - direct download and local copy
- media:
  - direct media URIs from Wix assets, not low-res thumbnails when avoidable
- contact:
  - page HTML plus internal component metadata to reconstruct form/map

### Migration workflow
1. Crawl all public routes and known assets into a migration manifest.
2. Freeze source inventory:
   - pages
   - posts
   - events
   - PDFs
   - media files
   - social links
3. Download original assets locally.
4. Convert page copy and post bodies into Markdown/MDX or structured content.
5. Normalize links to local `/media/...`.
6. Rebuild pages/components in Astro.
7. Add Decap CMS collections for maintainable content.
8. Add redirects from legacy Wix-style routes as needed.
9. Run manual QA for every page/content type.
10. Validate unresolved items with the user before final launch.

### Migration manifest should track
- source URL
- content type
- target route
- local asset path
...
- manual QA required
- approved by user
- notes on quirks/inconsistencies

## Deployment / CMS Direction
Chosen direction:
- Netlify-first
- Decap CMS

Implementation implications:
- Astro build output compatible with Netlify static hosting
- Decap CMS likely mounted at `/admin`
- Git-backed editorial workflow
- content in repo, not an external CMS by default

## Contact Form Status
This remains intentionally unresolved.

Known:
- current Wix site has a functional form
- current page also has a map section
- user explicitly chose `Decide Later`

Default planning assumption used earlier:
- if user does not decide before implementation, build the page structure in a way that can support a Netlify-compatible form backend

But this is still pending and should remain pending in future planning until confirmed.

## Information Still Needed From User
This is important. Future planning or implementation must keep these as pending requests rather than silently deciding.

### Required or strongly recommended confirmations
- original/source assets if available:
  - logo
  - banner files
  - board-member photos
  - skatepark photos
  - better originals than Wix-compressed copies
- confirmation that Instagram link is still correct
- confirmation of canonical YouTube link/account
- confirmation of:
  - address
  - phone
  - WhatsApp
  ...
  - donation IBAN
  - membership pricing
- GitHub repo owner/name/access expectations
- Netlify team/site access expectations
- final contact form backend choice
- whether Maps/YouTube should be:
  - embedded
  - click-to-load
  - replaced with plain links for privacy
- whether recurring Sunday Funday should become:
  - true recurring event data
  - or just an editorial static/recurring promo section
- any content intentionally to be removed and not migrated

## Recommended Verification Checklist Before Implementation
Before building, a cold-start continuation should confirm:
- repo still greenfield or understand any new changes
- user deployment decision still Netlify
- user content workflow still Decap CMS
- contact backend still undecided unless updated by user
- no newer instructions from user override prior decisions

## Recommended Implementation Phases Once Execution Is Allowed
This is not yet executed.

### Phase 1: Project setup
- scaffold Astro with `bun`
- configure TypeScript
- configure content collections
- add base layouts and design tokens
- set Netlify-compatible build config

### Phase 2: Content import foundation
- create migration manifest
- localize assets
- create content collection schemas
- import pages/posts/events/PDF metadata

### Phase 3: Core site shell
- header/nav/footer
- shared SEO metadata
- route structure
- redirects
...
- accessibility
- performance
- legal/data confirmation pass with user
...
## Final State of Planning So Far
Current planning is decision-complete enough on architecture, migration structure, and componentization to proceed into implementation.

What is still intentionally open:
- exact contact form backend
- canonical YouTube account/link
- user confirmation of live legal/contact/bank/membership data
- whether some current content should be dropped or updated rather than migrated as-is
