...
- During first Netlify setup, Identity invite/reset links were observed landing on public routes or `/admin/` without completing the CMS password flow. `BaseLayout` now forwards Netlify Identity token hashes (`invite_token`, `confirmation_token`, `recovery_token`, `email_change_token`, `access_token`, and `error`) from public pages to `/admin/`, including Netlify widget-compatible `#/...` hashes. `/admin/` now pauses Decap loading when an Identity token is present so the Netlify Identity widget can complete the invite/reset flow first, and it lets the widget's own token router select the invite/recovery screen instead of forcing the generic login modal.
